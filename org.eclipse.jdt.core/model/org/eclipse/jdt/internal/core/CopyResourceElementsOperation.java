...
public class CopyResourceElementsOperation extends MultiOperation implements SuffixConstants {
	 ...
	 */
	protected ArrayList<IJavaElement> createdElements;
	/**
	 ...
	 */
	protected Map<IJavaProject, JavaElementDelta> deltasPerProject = new HashMap<IJavaProject, JavaElementDelta>(1);
	/**
	...
	private JavaElementDelta getDeltaFor(IJavaProject javaProject) {
		JavaElementDelta delta = deltasPerProject.get(javaProject);
		if (delta == null) {
	...
	protected void processDeltas() {
		for (Iterator<JavaElementDelta> deltas = this.deltasPerProject.values().iterator(); deltas.hasNext();){
	...
	protected void processElements() throws JavaModelException {
		createdElements = new ArrayList<IJavaElement>(elementsToProcess.length);
		try {
