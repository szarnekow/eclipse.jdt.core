...
public class Extent {
	...
	public boolean isBoundingBoxFor(int left, int top, int right, int bottom) {
		return
				this.left <= left && this.top <= top && right <= this.getRight() && bottom <= this.getBottom() &&
				getWidthAsLong() == Math.max(1, (long)right - left) &&
				getHeightAsLong() == Math.max(1, (long)bottom - top);
	}
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + (int)(height >> 32);
		result = prime * result + (int)height;
		result = prime * result + left;
		result = prime * result + (int)(width >> 32);
	...
	public String toString() {
		return "drawit.shapegroups1.Extent[left="+left+", top="+top+", width="+width+", height="+height+"]";
	}
